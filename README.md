...
| ------------------ | --------------------- | ------------------------------ | ---------------------------------------- | --- | ---------------- |
| **/productDtos**      | Create a new productDto  | Retrieve all productDtos          | X                                        | X   |     X            |
| **/productDtos/:id**  | X                     | Retrieve details for productDto 1 | Update details of productDto 1 if it exists | X   | Remove productDto 1 |
...
- Imposer à l'utilisateur de se connecter pour accéder à l'API.
  ...
  * [POST] /accountDto -> Permet de créer un nouveau compte pour un utilisateur avec les informations fournies par la requête.   
    Payload attendu : 
    ```
    {
      username: string,
